...
import GoaSubCards from './GoaSubCards';
           ...
           <BHead />  
           {
            <div id="cmd" style={{display:"flex",flexWrap:"wrap",justifyContent:"flex-start",marginLeft:"70px"}}>
            {
                    this.state.places.map(x=>{
                        return(
                          <Link to={x.id}  key={x.id}><GoaSubCards img = {x.image}  p={x.category} tname={x.name} p2={"₹"+x.costPerHead} hrs="Duration" p3={x.duration+"hours"}/></Link>
                        )
                    })
                }
            </div>
